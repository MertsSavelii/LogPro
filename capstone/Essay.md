...
Главной оптимизацией, которую используют почти все программы, это alpha-beta отсичения, придуманный в 1962 тремя учёными: Алленом Ньюэллом, Гербертом Саймоном и Клиффордом Шоу. В основе этого алгоритма для сокращения числа узлов лежит простая идея, если у меня есть более хороший ход, то можно отсечь заведомо худшие, т.е. если можно установить, что, для определённой ветви значение оценивающей функции в любом случае хуже, чем вычисленное в предыдущем шаге для предыдущей ветви. Благодаря тому, что данный алгоритм не оценивает все варианты, он значительно лучше полного перебора. Суть алгоритма заключается в том, что задаётся две переменные alpha и beta, на каждом этапе игры отслеживались максимальные результаты для игрока и противника. Максимальная оценка хода для игрока - это alpha, ниже которого ход не имеет смысла, betta - максимально хорошоий ход для противника, хуже которого он не выберет (оценивающая функция выдаст для него минимальную оценку так, как это наиболее плохой ход для игрока). Если при анализе ход получает оценку выше beta — то текущий узел отсекается. Если оценка выше alpha — то alpha обновляется. Благодаря уменьшенному количеству рассматриваемых узлов можно повысить глубину поиска, что даёт приличную прибавку к "силе" программы.

Первые программы очень слабо играли начальный этап партии - дебют. На нём необходимо делать стратегические решения, которые отрязятся на все последующие ходы, а из-за сравнительно небольшой глубины поиска и оценивания трудно программе понимать правильный ли ход она сделала в начале. У людей теория дебютов была развито очень хорошо и многократно проанализировалась, были составлены, так называемые, дебютные книги. Багодаря долгому развитию дебютов любди их просто выучили и воспроизводили из памяти, вот и программам можно их вписать в память и выбирать одну из удачных коомбинаций с некоторой вероятностью.

В начале говорилось, что простой перебор всех положений слишком трудозатратный процес. Это правда, однако для ограниченного числа фигур прощитаны все ходы из всех положений. Полученные данные называют эндшпильной таблицей, потому что описывают этап игры, в котором осталось мало фигур - эндшпиль. Они могут занимать огромное количество памяти, но сильно помогли в изучении этого этапа игры. Приведу пару примеров: 
...
Написать подобную игру в шахматы на Prolog не так уж и сложно, если разобраться, стоит только захотеть. Для этого необхоимо разбить нашу задачу на несколько блоков.
...
* Третий - блок условий хода, то есть генератор ходов, который будет предлогать с помощью первого блока новые варианты действий, заполняя дерево ходов игры.

* Четвёртый - блок оценивающий правильность хода. Другими словами оценивающая фунция. Например хорошим считается ход, после которого нашему королю не будет поставлен мат. Благодаря этому и трретьему блоку, а так же блоку с оптимизациям, программа сможет выбирать правильный ход.

* Пятый - блоки пользователя и компьютера. Блок пользователя - это блок получения входных данных, проверка их на ликвидность, проверки на шах и можно уже отображать на экране смещение фигуры. Блок игры компьютера несколько сложнее. Он конкатенирует в себе вышеописанные блоки. Генерирует ход, оценивает его, выбирает самый выгодный проверяет на ликвидность и уже воспроизводит его.

* Дополнитеоьный блок в котором всякие вспомогательные придикаты на подобии предика выбора сторон или вывода доски с фигурами в терминал пользователя.

Это простейшее описание весьма слабой и глупой программы. Глупой от того, что выбирается любой ход, который не приводит к мату. Чтобы сделать игру интересне можно добавить всевозможных хитрых оптимизаций или книг дебюта с таблицами эндшпиля.

## Вывод

Подвдя итог можно сказать, что мы познакомились с парадигмой логического программирования, в частности с языком Prolog. Узнали его уникальные особенности, для чео и где применяется. Познакомились с теорией шахматных программ и описали алгоритм написания примитивной игры пользователя с компьютером в шахматы на языке Prolog, и выяснили какие есть недочеты у этого алгоритма и как можно было бы его улучшить.
